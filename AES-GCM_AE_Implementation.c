...
#define KEY_SIZE 32  // AES-128 için 16 byte
#define IV_SIZE 12   // GCM için standart IV boyutu
#define TAG_SIZE 16 // AES-GCM standard authentication tag size
...
// AES şifreleme fonksiyonu (AES-GCM)
...
int aes_gcm_encrypt(const unsigned char *plaintext, int plaintext_len,
    ...
    // Kimlik doğrulama etiketi oluştur
...
int aes_gcm_decrypt(const unsigned char *ciphertext, int ciphertext_len,
    ...
    // Ekstra veriyi (AAD) ekle
    if (1 != EVP_DecryptUpdate(ctx, NULL, &len, NULL, 0)) {
...
int main(int argc, char *argv[]) {
    //unsigned char key[KEY_SIZE] = {0x00}; // AES-128 key
    unsigned char key[KEY_SIZE] = "0123456789abcdef0123456789abcdef"; // AES-128 key
    ...
    int ciphertext_len = aes_gcm_encrypt(plaintext, plaintext_len, key, iv, ciphertext, tag);
    ...
    // Deşifreleme işlemi
    int decrypted_len = aes_gcm_decrypt(ciphertext, ciphertext_len, key, iv, tag, decrypted);
